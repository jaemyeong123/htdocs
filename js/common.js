...
function modal_open_1(){
  $(".md_overlay_1").css("visibility", "visible").animate({opacity: 1}, 500);
  $(".modal_1").css({display: "block"}).animate({top: "20%", opacity: 1}, 500);
}

function modal_close_1(){
  $(".modal_1").animate({top: "10%", opacity: 0}, 500);
  setTimeout(function(){
    $(".modal_1").css({display: "none"});
  },500);
  setTimeout(function(){
    $(".md_overlay_1").css("visibility", "hidden").animate({opacity: 0}, 500);
  },600);
  ...
  $.ajax({
    ...
    success     : function(img_list){
      ...
      for(var i = 0; i < img_list.length; i++){
        str="<li id='id_file_"+i+"_"+file_cnt+"' class='flie_li' style='display:inline-block;width:350px;'>"
        if(file_type !="file"){
          str+="<img class='preview_img'  style='width:300px;' src='"+img_list[i].path+"'>";
        }else{
          str+= img_list[i].orig_name;
        }

        str+= "<a href='javascript:void(0)' style='width:15px;cursor:pointer' onclick=\"file_upload_remove('"+i+"_"+file_cnt+"');\"/><img src='/images/img_delete.png'></a>";
