...
Class Pg_function {
   ...
   function inicis_cancel($pg_tid,$msg) {
		header("Content-type: text/html; charset=utf-8");

		$clientIp ="175.126.62.58";//서버ip
		$current_time =date("YmdHis");
		$mid=INICIS_MID;

		$temp_data =INICIS_KEY."Refund"."Card".$current_time.$clientIp.INICIS_MID.$pg_tid;
		$hashData = hash('sha512', $temp_data);


		$ch = curl_init('https://iniapi.inicis.com/api/v1/refund'); // Initialise cURL
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('application/x-www-form-urlencoded;charset=utf-8' )); // Inject the token into the header
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_POST, 1); // Specify the request method as POST
		curl_setopt ($ch, CURLOPT_POSTFIELDS,  "type=Refund&paymethod=Card&timestamp=$current_time&clientIp=$clientIp&mid=$mid&tid=$pg_tid&msg=$msg&hashData=$hashData"); // Post 값 Get 방식처럼적는다.
