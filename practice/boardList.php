                ...
                <div class="content-wrapper" style="box-shadow: 5px 15px 30px 0.5px #E6E6E6;margin-left: 25px;box-sizing:border-box;height:600px;width:1100px;">
                    ...
                    <div class="content-list" style="margin-left: 50px;margin-top: 50px;box-sizing:border-box;height:600px;width:1000px;">
                        ...
                        <div style="margin-bottom: 10px;height:auto;overflow: hidden;width:1000px;box-sizing:border-box;border-bottom:1px solid #E6E6E6;">
                            ...
                            if ($count<1) {
                                // 테이블에 row가 없으면 프린트 처리
                            } else {
                                while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
                                    ?>
                                    <div OnClick="location.href ='http://localhost/practice/boardDetail.php?board_id=<?=$row['board_id']?>'" style="text-align:center;line-height:60px;height:60px;width:100px;float:left;font-size:1.3em;"><?=$row['board_id']?></div>
                                    <div OnClick="location.href ='http://localhost/practice/boardDetail.php?board_id=<?=$row['board_id']?>'" style="text-align:center;line-height:60px;height:60px;width:500px;float:left;font-size:1.3em;"><?=$row['title']?></div>
                                    <div OnClick="location.href ='http://localhost/practice/boardDetail.php?board_id=<?=$row['board_id']?>'" style="text-align:center;line-height:60px;height:60px;width:400px;float:left;font-size:1.3em;"><?=$row['regdate']?></div>
                                    <?php
                                }
                            }
                            ?>
                        </div>
                        <div OnClick="location.href ='http://localhost/practice/boardRegist.php'" style="float:right;margin-right: 10px; text-align:center; line-height:45px; font-size:1.0em; border-radius:5px;width:100px;height:45px;background-color:#0B3861;color:#FFFFFF;">글쓰기</div>
